package LegalPublisher::AmericanLegal;
use strict;
use warnings;
...
sub subdivisions {
    ...
    my $subdivisions_url = $config->{'legal publisher'}->{'american legal'}->{'origin'};

    # We need a robot.
    my $mech = WWW::Mechanize->new();
    $mech->get($subdivisions_url);

    # Now let's look for the listing that has the $filter...
    $mech->follow_link(url_regex => qr/http:\/\/www\.amlegal\.com\/codes\/${filter}/);

    my $page = $mech->content();
    while ($page =~ /<h2><a href="(.+?)">(.+?)<\/a><\/h2>/g) {
